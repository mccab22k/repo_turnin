...
def build_packet():
	...
	# Append checksum to the header. (see skeleton from week 5)
	...
	for ttl in range(1,MAX_HOPS):
		...
		for tries in range(TRIES):
			...
			try:
				...
				types,codes,checksum,pid,seq,=struct.unpack("bbHHh,icmp_header", headericmp)
				#Fill in end
				...
				if types == 11:
					...
					tracelist1.insert(-1,str(int((timeReceived-t)*1000)))
					tracelest1.insert(-1,addr[0])
					tracelist2.append(tracelist1)
					#Fill in end
				elif types == 3:
					bytes = struct.calcsize("d")
